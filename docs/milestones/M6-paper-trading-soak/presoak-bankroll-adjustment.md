...
- The strategy must have no signals, fills, or positions.
...
a lower target creates a withdrawal. The operation then updates
`initial_deposit_cents` and `bankroll_hwm_cents` to the requested starting
amount and writes a `set_starting_bankroll` audit event.
