...
def _location_settings(shared_url: str) -> Settings:
    return Settings(
        REQUIRE_DBS=False,
...
def test_seed_locations_is_idempotent(per_env_sqlite_urls: tuple[str, str]) -> None:
    ...
    settings = _location_settings(shared_url)
...
def test_lifespan_seeds_reference_locations_on_startup(
    ...
    with shared_session(settings) as session:
        count = session.scalar(select(func.count()).select_from(ReferenceLocationRow))
        seeded_ids = {row.id for row in session.scalars(select(ReferenceLocationRow)).all()}

    assert count == len(SEED_LOCATIONS)
    assert seeded_ids == {loc[0] for loc in SEED_LOCATIONS}
