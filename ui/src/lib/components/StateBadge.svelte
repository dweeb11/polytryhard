<script lang="ts">
	import { strategyStateLabel } from '$lib/utils';
...
<span
	class="inline-flex rounded border px-2 py-0.5 text-xs font-medium capitalize {colors[state] ??
		'bg-slate-800 text-slate-300 border-slate-600'}"
>
	{strategyStateLabel(state)}
</span>
